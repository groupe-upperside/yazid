  ...
  return (
    <>
      <div className="grid">
        ...
        <div
          className={`hidden lg:grid gap-8 md:grid-cols-3 lg:grid-cols-${slice.primary.number_per_row - 1} ${getGridColsClass(slice.primary.number_per_row)}`}
        >
          {slice.primary.images.map((item) => (
            <div>
              <PrismicNextImage className="w-full object-cover aspect-square" field={item.image} />
            </div>
          ))}
        </div>
...
const OnlyImagesGrid = ({ slice }: ImageGridProps): JSX.Element => {
  return (
    <div
      className={`grid gap-8 md:grid-cols-3 lg:grid-cols-${slice.primary.number_per_row - 1} ${slice.variation === 'imagesOnlyBgWhite' ? 'bg-white' : 'bg-[#F7F4EF]'} ${getGridColsClass(slice.primary.number_per_row)}`}
    >
      {slice.primary.images.map((item) => (
        <div>
          <PrismicNextImage className="w-full object-cover aspect-square" field={item.image} />
...
const ImageGrid = ({ slice }: ImageGridProps): JSX.Element => {
  return (
    <section
      ...
      className={`${slice.variation === 'imagesOnlyBgWhite' || slice.variation === 'default' ? 'bg-white' : 'bg-[#F7F4EF]'} p-20 xl:p-32 font-avenir tracking-widest`}
    >
