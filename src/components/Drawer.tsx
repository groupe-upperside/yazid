            ...
            <div className="overflow-y-auto py-6 md:py-4">
              <ul className="hidden space-y-4 font-medium xl:block">
                {meta.data.sidebar.map(({ link, label }) => (
                  <li key={label}>
                    <PrismicNextLink
                      className="block py-2 pl-3 pr-4 text-[0.89rem] font-semibold uppercase tracking-widest lg:p-0"
                      field={link}
                    >
                      {label}
                    </PrismicNextLink>
          ...
          <li key={label}>
            {custom_action && custom_action === 'open_drawer' ? (
              <button
                ...
                onClick={() => setOpenDrawer(!openDrawer)}
                type="button"
                className="font-base block py-2 pl-3 pr-4 text-[0.89rem] uppercase tracking-widest lg:border-0 lg:p-0"
              >
                {label}
              </button>
            ) : (
              <PrismicNextLink
                className="block py-2 pl-3 pr-4 text-[0.89rem] uppercase lg:border-0 lg:p-0"
                field={link}
              >
                {label}
              </PrismicNextLink>
